...
const ResizableScrollableTabs = () => {
  ...
  // TabsのDOMを指す
  // const refTabsArea = useRef<HTMLDivElement>(null);
  const refTabsArea = useRef<HTMLDivElement>();
  ...
  useEffect(() => {
    if (refTabsArea.current) {
      setScrollableWidth(refTabsArea.current.getBoundingClientRect().width);
      setScrollWidth(refTabsArea.current.scrollWidth);
    }
  }, []);
  ...
  const incrementTab = (flag: boolean) => {
    ...
    else setNumberOfTabs(numberOfTabs);
  };
  ...
  // Tabsをスクロールさせる
  if (refTabsArea.current) {
    refTabsArea.current.scrollLeft = scrollLeft;
  }
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Tabs _ref={refTabsArea} numberOfTabs={numberOfTabs} />
          <SimpleSlider
